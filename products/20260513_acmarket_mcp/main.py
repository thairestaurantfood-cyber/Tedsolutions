#!/usr/bin/env python3
"""
AC Market MCP Server - under 200 lines
Exposes service marketplace as MCP server with tools:
- list_service, browse_services, buy_service
"""
...
def browse_svc(unsold=True):
    with sqlite3.connect(MARKET_DB) as conn:
        ...
        out = [f"Available Services:" if unsold else "All Services:", "-"*60]
        for r in rows:
            if unsold:
...
def serve():
    ...
    init_wallet()
    init_market()
    server = Server("ac-market")
    
    @server.list_tools()
    async def list_tools():
        return [
            Tool(name="list_service", description="List a service for sale",
                 inputSchema={"type":"object","properties":{"seller":{"type":"string"},"name":{"type":"string"},"description":{"type":"string"},"price":{"type":"integer"}},"required":["seller","name","description","price"]}),
            Tool(name="browse_services", description="Browse available services",
                 inputSchema={"type":"object","properties":{"unsold":{"type":"boolean"}}}),
            Tool(name="buy_service", description="Buy a service",
                 inputSchema={"type":"object","properties":{"buyer":{"type":"string"},"service_id":{"type":"integer"}},"required":["buyer","service_id"]})
        ]
    
    @server.call_tool()
    async def call_tool(name, args):
        if name == "list_service": 
            return [TextContent(type="text", text=list_svc(args.get("seller",""), args.get("name",""), args.get("description",""), int(args.get("price",0))))]
        if name == "browse_services": 
            return [TextContent(type="text", text=browse_svc(args.get("unsold", True)))]
        if name == "buy_service": 
            return [TextContent(type="text", text=buy_svc(args.get("buyer",""), int(args.get("service_id",0))))]
